...
public class ItemCoraliumBow extends ItemEffectBow {
	...
	protected EntityArrow getFiredArrow(EntityPlayer player, World world, float f) {
		return new EntityCoraliumArrow(world, player, f * 2.0F);
	}

	@Override
	public boolean getIsRepairable(ItemStack tool, ItemStack repairMaterial) {
		return AbyssalCraft.Cingot == repairMaterial.getItem() ? true : super.getIsRepairable(tool, repairMaterial);
	}
