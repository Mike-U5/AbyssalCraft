...
public class NecronomiconInfusionRitual extends NecronomiconCreationRitual {
	 ...
	 * @param offerings       Components used to perform the ritual, are consumed
	 *                        afterwards
	 */
	public NecronomiconInfusionRitual(String unlocalizedName, int bookType, int dimension, float requiredEnergy, boolean remnantHelp, ItemStack item, Object sacrifice, Object... offerings) {
	...
	protected void completeRitualServer(World world, int x, int y, int z, EntityPlayer player) {
		if (canCompleteRitual(world, x, y, z, player))
			super.completeRitualServer(world, x, y, z, player);
	}

	@Override
	protected void completeRitualClient(World world, int x, int y, int z, EntityPlayer player) {
		if (canCompleteRitual(world, x, y, z, player))
			super.completeRitualClient(world, x, y, z, player);
	}
}
